...
public:
	...
	//for my method
	void UpdateIngressWaitingtime(uint32_t portin, uint32_t portout, uint32_t qIndex, uint32_t rate);
	void RemoveFromIngressWaitingtime(uint32_t portin, uint32_t portout, uint32_t qIndex, uint32_t rate);
	uint64_t CongestionAwareBM(uint32_t port, uint32_t qIndex, std::string inout, uint32_t type);
	uint64_t ActiveBM(uint32_t port, uint32_t qIndex, std::string inout, uint32_t type);
	void SetAlg(uint32_t alg);
	uint64_t DynamicThreshold2(uint32_t port, uint32_t qIndex, std::string inout, uint32_t type);
	...
	uint32_t packetcounter[pCnt][qCnt];
	double totalwaitingtime[pCnt][qCnt];
	double lastupdatetime[pCnt][qCnt];
